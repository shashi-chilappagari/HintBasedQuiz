...
function loadQuestion(questionIndex,optionOrder){
	...
	for (var i=0;i<q.options.length;i++){
		...
		if(q.answerImages)
		{
			answerOption.style.textAlign="center";
			if(q.answerImages[optionOrder[i]])
			{
				imgsrc="img/"+q.answerImages[optionOrder[i]];
		...
		textNumber.innerHTML='<input type="radio" id="option'+(i+1)+'" name="option"'+' value='+ (i+1)+ ' /><label for="option'+(i+1)+'">'+q.options[optionOrder[i]]+'</label>';
